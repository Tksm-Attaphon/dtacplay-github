...
-(void)updateContent{
    ...
    [view setFrame:CGRectMake(0,0, 200, 30)];
    view.tag = 1;
    view.delegate = self;
    ...
    switch (article.subCateID) {
        case NEWS_WIKI:
            url = [NSString stringWithFormat: @"%@wiki/%@?m=share",DOMAIN_WEBSITE,article.contentID];
            
            break;
        case NEWS_HOT_NEWS:
            url = [NSString stringWithFormat: @"%@hot/%@?m=share",DOMAIN_WEBSITE,article.contentID];
            break;
        case NEWS_INTER_NEWS:
            url = [NSString stringWithFormat: @"%@hot/%@?m=share",DOMAIN_WEBSITE,article.contentID];
            break;
        case NEWS_FINANCE:
            url = [NSString stringWithFormat: @"%@finance/%@?m=share",DOMAIN_WEBSITE,article.contentID];
            break;
        case NEWS_TECHNOLOGY:
            url = [NSString stringWithFormat: @"%@technology/%@?m=share",DOMAIN_WEBSITE,article.contentID];
            break;
        case NEWS_LOTTO:
            url = [NSString stringWithFormat: @"%@lotto/%@?m=share",DOMAIN_WEBSITE,article.contentID];
            break;
        default:
            break;
    }
    ...
    if(article.feedID == nil){
        if(article.descriptionContent != nil){
            ...
            if(article.subCateID != NEWS_WIKI)
                ...
                [textView setFont:[UIFont fontWithName:@"HelveticaNeue" size:IDIOM==IPAD ? 16: 14]];
            textView.tag =80;
...
-(void)setFrameContent{
    ...
    [contentView setFrame:CGRectMake(0, titleHeader.frame.origin.y+titleHeader.frame.size.height+(IDIOM == IPAD ? 20 : 10), scrollViewWidth, y-5)];
